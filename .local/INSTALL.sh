wget https://github.com/getzola/zola/releases/download/v0.20.0/zola-v0.20.0-x86_64-unknown-linux-gnu.tar.gz
tar xvzf zola-v0.20.0-x86_64-unknown-linux-gnu.tar.gz
sudo mv zola /usr/local/bin 
mv zola-v0.20.0-x86_64-unknown-linux-gnu.tar.gz .local

wget https://github.com/getzola/zola/releases/download/v0.22.1/zola-v0.22.1-x86_64-unknown-linux-gnu.tar.gz
tar xvzf zola-v0.22.1-x86_64-unknown-linux-gnu.tar.gz
sudo mv zola /usr/local/bin 
mv zola-v0.22.1-x86_64-unknown-linux-gnu.tar.gz .local
mv artifacts .local